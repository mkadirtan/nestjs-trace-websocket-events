...
export class TraceableIoAdapter extends AbstractWsAdapter {
  public create(
    port: number,
    ...
    handlers.forEach(({ message, callback }) => {
      const source$ = fromEvent(socket, message).pipe(
        mergeMap((payload: any) => {
          ...
          return transform(callback(data, ack, requestId)).pipe(
  ...
  public mapPayload(payload: unknown): { data: any; ack?: AckFunction } {
    if (!Array.isArray(payload)) {
      if (isFunction(payload)) {
        return { data: undefined, ack: payload as AckFunction };
      }
      return { data: payload };
    }
